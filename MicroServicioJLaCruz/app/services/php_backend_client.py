import requests
import os
from dotenv import load_dotenv

load_dotenv()
MAIN_SYSTEM_API_URL = os.getenv("MAIN_SYSTEM_API_URL")

def obtener_catalogo():
    """
    Hace una petición HTTP al sistema principal (PHP) para obtener 
    los productos y servicios actualizados de la base de datos.
    """
    try:
        headers = {"X-Internal-Token": "JLacruz2026Secure"}
        # Aseguramos que la URL no tenga la accion en GET y la enviamos por POST
        url_limpia = MAIN_SYSTEM_API_URL.replace("&accion=obtenerCatalogo", "")
        response = requests.post(url_limpia, headers=headers, data={"accion": "obtenerCatalogo"})
        response.raise_for_status()
        data = response.json()
        if data.get("status") == "success":
            return data.get("data", {})
        else:
            print("Error en el formato de respuesta del catálogo:", data)
            return None
    except Exception as e:
        print(f"Error al conectar con el sistema principal: {e}")
        return None
