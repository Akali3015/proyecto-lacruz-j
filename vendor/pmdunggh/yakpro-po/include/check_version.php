...
$php_parser_branch = null;
if (isset($t_composer->extra) && isset($t_composer->extra->{'branch-alias'}) && isset($t_composer->extra->{'branch-alias'}->{'dev-master'})) {
  $php_parser_branch = $t_composer->extra->{'branch-alias'}->{'dev-master'};
} else {
  $gitHeadFile = "$php_parser_path/.git/HEAD";
  if (is_file($gitHeadFile)) {
    $gitHead = trim(file_get_contents($gitHeadFile));
    if (preg_match('/^ref: refs\/heads\/(.+)$/', $gitHead, $matches)) {
      $php_parser_branch = $matches[1];
    }
  }
  if ($php_parser_branch === null) {
    $php_parser_branch = '4.x';
  }
}
